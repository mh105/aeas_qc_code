...
if isfile(fullfile(fpath, fname))
    ...
    idx = contains(event_array, {'Impedance', 'Amplifier disconnected',...
    ...
    task_codes = zeros(1, length(partitions));
    for ii = 1:length(partitions)
        ...
        elseif isempty(task_code_idx)
            ...
            assert(isscalar(current_task_code), 'Multiple task codes within a partition. Please check this!')
            ...
            tmp_line{2} = [tmp_line{1}, ' (previous)'];
            tmp_line{3} = NaN;
        ...
        task_codes(ii) = str2double(current_task_code);
    end
    ...
    disp(['Checking the .cnt file segment of ', '[ emg_artifact ]', '...', newline])
    cnt_check_segment_issues(EEG_tasks{1})

    disp(['Checking the .cnt file segment of ', '[ bluegrass_memory ]', '...', newline])
    cnt_check_segment_issues(EEG_tasks{2})

    disp(['Checking the .cnt file segment of ', '[ auditory_oddball ]', '...', newline])
