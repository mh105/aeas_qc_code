function [ num_rep, edf_fn ] = file_check_psychopy_files(fpath, subID, task_name)
% find all files under the directory
task_fns = {dir(fpath).name};

% filter down to only files containing the task_name string
task_fns = task_fns(contains(task_fns, task_name) & contains(task_fns, subID));
...
if ~isempty(task_fns)
    ...
    if length(log_fn) > 1
        disp(['Multiple session numbers for task: [ ', task_name, ' ]'])
        disp(log_fn)
    end
    log_fn = log_fn{1};
    if ~contains(log_fn, [task_name, '_001'])
        disp(['Session number is not 001 for task [ ', task_name, ' ]:', log_fn])
    end
    ...
    if length(psydat_fns) > 1
        ...
        % disp(['The task was started ', num2str(num_rep), ' times: [ ', task_name, ' ]'])
    else
        num_rep = 1;
    ...
    disp(['Missing PsychoPy files for task: [ ', task_name, ' ]'])
    num_rep = 0;
    edf_fn = [];
